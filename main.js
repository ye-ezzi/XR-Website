...
class ModelViewer {
    ...
    startMovementAnimation() {
        ...
        // 애니메이션 로드 완료 후 SVG 속성 강제 조정
        this.movementAnimation.addEventListener('DOMLoaded', () => {
            const svg = container.querySelector('svg');
            if (svg) {
                svg.setAttribute('viewBox', '0 0 1920 1080'); // 강제 viewBox 설정
        ...
        // 10초 후 자동으로 마지막 프레임에서 정지
        setTimeout(() => {
            if (this.movementAnimation) {
                this.movementAnimation.goToAndStop(this.movementAnimation.totalFrames - 1, true);
            }
        }, 10000); // 10초 = 좌로 4초 + 우로 4초 + 복귀 2초
        
        console.log('Movement animation started');
    }
